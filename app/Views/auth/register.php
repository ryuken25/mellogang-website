...
<div class="auth-wrap">
  ...
  <div class="card">
    ...
    <?php $googleConfigured = ! empty($googleOn); ?>
    <a class="btn-google" href="<?= $googleConfigured ? site_url('auth/google/redirect') : 'javascript:void(0)' ?>"
       <?= $googleConfigured ? '' : 'onclick="alert(\'Login Google belum dikonfigurasi. Tambahkan GOOGLE_CLIENT_ID & GOOGLE_CLIENT_SECRET di .env.\')"' ?>>
    ...
    <?php if (! $googleConfigured): ?>
      <div class="muted" style="font-size:11px;margin-top:6px;text-align:center;line-height:1.4;">
        Login with Google is wired but not active until you set <code style="background:var(--bg-elev);padding:1px 4px;border-radius:4px;">GOOGLE_CLIENT_ID</code> &amp; <code style="background:var(--bg-elev);padding:1px 4px;border-radius:4px;">GOOGLE_CLIENT_SECRET</code> in <code style="background:var(--bg-elev);padding:1px 4px;border-radius:4px;">.env</code>.
      </div>
    <?php endif; ?>
