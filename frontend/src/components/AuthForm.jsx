...
import { demoUsers, setDemoSession } from '../data/demoUserData'
...
export default function AuthForm({ mode }){
  ...
  function submit(e){
    const demo = demoUsers.find(u => u.email.toLowerCase() === form.email.toLowerCase() && u.password === form.password)
    if (!isSignup && demo) { e.preventDefault(); setDemoSession(demo); navigate('/status-pesanan'); return }
    if(!valid){ e.preventDefault(); setError(isSignup && form.password!==form.password_confirm ? 'Password confirmation must match.' : 'Isi email valid dan password minimal 5 karakter. Demo: dummy@dummy.com / dummy'); return }
    if(!action || action === (isSignup?'/register':'/login')) { e.preventDefault(); setError('Backend auth belum terhubung. Pakai demo login dummy@dummy.com / dummy atau set VITE_BACKEND_BASE_URL.'); return }
    setLoading(true)
  }
